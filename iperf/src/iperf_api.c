...
iperf_new_stream(struct iperf_test *test, int s)
{
...
#else
    char template[] = "/data/data/com.example.wifitest/cache/iperf3.XXXXXX";
#endif
    h_errno = 0;

	//iperf_errexit(test, " __func__start  template = %s",template);
    sp = (struct iperf_stream *) malloc(sizeof(struct iperf_stream));
    if (!sp) {
		iperf_errexit(test," __func__ 1111 ");
    ...
    if (!sp->result) {
        free(sp);
        i_errno = IECREATESTREAM;
		iperf_errexit(test," __func__ 222");
        return NULL;
    }
    ...
    if (sp->buffer_fd == -1) {
        ...
        free(sp->result);
        free(sp);
		iperf_errexit(test, " __func__ 333  template = %s",template);
        return NULL;
    }
    if (unlink(template) < 0) {
        i_errno = IECREATESTREAM;
		iperf_errexit(test," __func__ 555i");
        free(sp->result);
        free(sp);
        return NULL;
    }
    if (ftruncate(sp->buffer_fd, test->settings->blksize) < 0) {
        i_errno = IECREATESTREAM;
		iperf_errexit(test," __func__666 ");
        free(sp->result);
        free(sp);
        return NULL;
    }
    ...
    if (sp->buffer == MAP_FAILED) {
        i_errno = IECREATESTREAM;
		iperf_errexit(test," __func__ 777");
        free(sp->result);
        free(sp);
        return NULL;
    }
